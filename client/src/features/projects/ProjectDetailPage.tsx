...
export default function ProjectDetailPage() {
  ...
  async function loadProject(pid: string) {
    try {
      const res = await projectsApi.getById(pid);
          ...
          {project.localPath && (
            <button
              onClick={() => projectsApi.open(project.id)
                ...
                .catch((e: any) => message.warning(e.response?.data?.error?.message))}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: 8,
...
function ReposTab({ projectId, repos, onRefresh }: { projectId: string; repos: any[]; onRefresh: () => void }) {
  ...
  const handleAdd = async (values: any) => {
    try {
      ...
      message.error(err.response?.data?.error?.message || '关联失败');
    }
  };
  ...
  const handleSync = async (repoId: string) => {
    ...
    try {
      ...
      message.error(err.response?.data?.error?.message || '同步失败');
    } finally {
      setSyncing(null);
    }
  };
...
function ProjectTimelineTab({ projectId }: { projectId: string }) {
  ...
  async function loadTimeline() {
    try {
      const res = await timelineApi.byProject(projectId, { limit: 50 });
