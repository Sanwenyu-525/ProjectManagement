...
export function makeDevhubTheme(isDark: boolean) {
  return EditorView.theme({
    ...
    '.cm-activeLineGutter': {
      backgroundColor: 'rgba(0, 107, 95, 0.06)',
      color: 'var(--ws-text-secondary)',
    },
    '.cm-activeLine': {
      backgroundColor: 'rgba(0, 107, 95, 0.04)',
    },
    '.cm-selectionBackground': {
      backgroundColor: 'rgba(0, 107, 95, 0.14) !important',
    },
    '&.cm-focused .cm-selectionBackground': {
      backgroundColor: 'rgba(0, 107, 95, 0.20) !important',
    },
    '.cm-matchingBracket': {
...
export const devhubHighlight = EditorView.baseTheme({
  '.ͼb': { color: '#8455ef' },     // keyword (secondary-container purple)
  '.ͼc': { color: '#6c7a77', fontStyle: 'italic' }, // comment (outline gray)
  '.ͼd': { color: '#16bb83' },     // string (tertiary-container green)
  '.ͼe': { color: '#00423b' },     // number (on-primary-container)
  '.ͼi': { color: '#006b5f' },     // type name (primary teal)
  '.ͼg': { color: '#3c4947' },     // variable name (on-surface-variant)
});
