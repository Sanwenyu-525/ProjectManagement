...
import type { ProjectWithStats, ActivityLog, ProjectHealthResult, Workspace } from '../../../types';
...
interface ProjectWithCount extends ProjectWithStats {
  _count?: { documents?: number; tasks?: number };
  remoteRepos?: unknown[];
}

interface HealthWithScore extends ProjectHealthResult {
  healthStatus?: string;
  healthScore?: number;
}
...
export default function DataScreenPage() {
  ...
  const { data: timelineData, isLoading: timelineLoading, refetch: refetchTimeline } = useTimeline({ limit: 100 });
  const { data: healthData = [], isLoading: healthLoading, refetch: refetchHealth } = useAllHealth();
  const { data: workspaceData = [], isLoading: workspacesLoading, refetch: refetchWorkspaces } = useWorkspaces();

  const projects = (projectsData ?? []) as ProjectWithCount[];
  const activityLogs = (timelineData ?? []) as ActivityLog[];
  const healthResults = healthData as HealthWithScore[];
  const workspaces = workspaceData as Workspace[];

  const loading = projectsLoading || timelineLoading || healthLoading || workspacesLoading;
  ...
  const healthScoreDist = useMemo(() => {
    ...
    return [
      ...
      { name: '未检测', value: projects.length - healthResults.length, itemStyle: { color: tc.textMuted } },
    ].filter(d => d.value > 0);
  }, [healthResults, projects.length]);

  // 平均健康评分
  const avgHealthScore = useMemo(() => {
    const scores = healthResults.filter((h) => h.healthScore != null).map((h) => h.healthScore as number);
  ...
  const workspaceDist = useMemo(() => {
    ...
    const items = workspaces
      ...
      .filter((d) => d.value > 0);
    const assignedCount = workspaces.reduce((s: number, w) => s + ((w.projectCount as number) || 0), 0);
              ...
              <List.Item style={{ borderBottom: '1px solid var(--color-border-subtle)', padding: '8px 0' }}>
                <div style={{ width: '100%' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Text style={{ color: 'var(--color-text-description)', fontSize: 11 }}>{new Date(log.createdAt).toLocaleString('zh-CN')}</Text>
                  </div>
                  <div style={{ color: 'var(--color-text-primary)', fontSize: 13, marginTop: 4 }}>{log.projectName}</div>
                </div>
              </List.Item>
            )}
          />
