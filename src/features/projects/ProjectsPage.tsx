...
import { Button, Input, Select, Tag, Modal, Form, Empty, Spin, Table, InputNumber } from 'antd';
...
export default function ProjectsPage() {
  ...
  // Stable key: only changes when the actual project list changes, not on every react-query refetch
    ...
    <div style={{ padding: 'var(--layout-container-padding)', maxWidth: 1600, margin: '0 auto' }}>
          ...
          <div style={{
            ...
            {(['grid', 'list', 'kanban'] as const).map(mode => (
              <button
                ...
                onMouseEnter={e => {
                  if (viewMode !== mode) e.currentTarget.style.background = 'var(--md-surface-container-high)';
                }}
                onMouseLeave={e => {
      ...
      {loading ? (
        ...
        <ProjectGridView
          ...
          density={density}
          navigate={navigate}
          getStatusColorVar={getStatusColorVar}
